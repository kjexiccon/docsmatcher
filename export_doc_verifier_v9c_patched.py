...
st.set_page_config(page_title="SHEESH SPICES Verifier – v9c FINAL", layout="wide")
...
def export_pdf(df_rows):
    ...
    for _, row in df_rows.iterrows():
        line = re.sub(r'[^\x00-\xFF]+', '', line)  # remove non-latin1
        line = f"{row.get('Field', row.get('Product', ''))} → {row['Match' if 'Match' in row else 'Status']} in {row['Found in' if 'Found in' in row else 'Doc']}"
